...
/// with open('example.npy', 'wb') as file:
///     np.save(file, torch_tensor.numpy())
ov::Tensor from_npy(const std::filesystem::path& npy) {
...
ov::Tensor HD_transform(const ov::Tensor& uint8, size_t num_crops) {
    ...
    ov::Tensor restored = tensor_from_npy("./i/resize.npy");
    ov::Shape expanded_dims = restored.get_shape();
    expanded_dims.insert(expanded_dims.begin(), 1);
    ov::Tensor expanded{ov::element::u8, expanded_dims};
    std::copy_n(restored.data<uint8_t>(), restored.get_size(), expanded.data<uint8_t>());
    return padding_336(expanded);
}
...
std::tuple<ov::Tensor, ImageSize> get_pixel_values_phi3_v(const ov::Tensor& image, const ProcessorConfig& config) {
    ...
    global_image = tensor_from_npy("./i/interpolate.npy");
